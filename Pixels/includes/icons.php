<?php
function icon_svg(
    string $class,
    array $parts
): string {
    return '<svg class="' .
        $class .
        '" viewBox="0 0 24 24" ' .
        'fill="none" ' .
        'stroke="currentColor" ' .
        'stroke-width="2" ' .
        'stroke-linecap="round" ' .
        'stroke-linejoin="round" ' .
        'aria-hidden="true">' .
        implode("", $parts) .
        "</svg>";
}

function icon_tree_pine(
    string $class = ""
): string {
    return icon_svg($class, [
        '<path d="M12 2L8 7h8l-4-5z"/>',
        '<path d="M12 7l-5 7h10l-5-7z"/>',
        '<path d="M12 14l-6 8h12l-6-8z"/>',
    ]);
}

function icon_pointer(
    string $class = ""
): string {
    return icon_svg($class, [
        '<path d="M3 3l7.5 17L13 13l7-2-17-8z"/>',
    ]);
}

function icon_dollar(
    string $class = ""
): string {
    return icon_svg($class, [
        '<line x1="12" y1="1" x2="12" y2="23"/>',
        '<path d="' .
            'M17 5H9.5a3.5 3.5 0 0 0 0 7' .
            'H14a3.5 3.5 0 0 1 0 7H6"/>',
    ]);
}

function icon_sparkles(
    string $class = ""
): string {
    return icon_svg($class, [
        '<path d="' .
            'M9 3l1.5 3L14 7.5 10.5 9 ' .
            '9 12 7.5 9 4 7.5 7.5 6z"/>',
        '<path d="' .
            'M18 13l1 2 2 1-2 1-1 2' .
            '-1-2-2-1 2-1z"/>',
        '<path d="' .
            'M6 14l.75 1.5L8.5 16' .
            'l-1.75.5L6 18l-.75-1.5' .
            'L3.5 16l1.75-.5z"/>',
    ]);
}

function icon_arrow_right(
    string $class = ""
): string {
    return icon_svg($class, [
        '<path d="M5 12h14"/>',
        '<path d="M13 5l7 7-7 7"/>',
    ]);
}

function icon_arrow_left(
    string $class = ""
): string {
    return icon_svg($class, [
        '<path d="M19 12H5"/>',
        '<path d="M11 19l-7-7 7-7"/>',
    ]);
}

function icon_shield(
    string $class = ""
): string {
    return icon_svg($class, [
        '<path d="' .
            'M12 2l7 4v6c0 5-3.5 9.5' .
            '-7 10-3.5-.5-7-5-7-10V6l7-4z"/>',
    ]);
}

function icon_check_circle(
    string $class = ""
): string {
    return icon_svg($class, [
        '<circle cx="12" cy="12" r="10"/>',
        '<path d="M9 12l2 2 4-4"/>',
    ]);
}

function icon_users(
    string $class = ""
): string {
    return icon_svg($class, [
        '<path d="' .
            'M17 21v-2a4 4 0 0 0-4-4' .
            'H5a4 4 0 0 0-4 4v2"/>',
        '<circle cx="9" cy="7" r="4"/>',
        '<path d="' .
            'M23 21v-2a4 4 0 0 0-3-3.87"/>',
        '<path d="' .
            'M16 3.13a4 4 0 0 1 0 7.75"/>',
    ]);
}

function icon_building(
    string $class = ""
): string {
    return icon_svg($class, [
        '<rect x="3" y="3" width="18" ' .
            'height="18" rx="2" ry="2"/>',
        '<path d="M9 3v18"/>',
        '<path d="M15 3v18"/>',
        '<path d="M3 9h18"/>',
        '<path d="M3 15h18"/>',
    ]);
}

function icon_calendar(
    string $class = ""
): string {
    return icon_svg($class, [
        '<rect x="3" y="4" width="18" ' .
            'height="18" rx="2" ry="2"/>',
        '<line x1="16" y1="2" x2="16" y2="6"/>',
        '<line x1="8" y1="2" x2="8" y2="6"/>',
        '<line x1="3" y1="10" x2="21" y2="10"/>',
    ]);
}

function icon_facebook(
    string $class = ""
): string {
    return icon_svg($class, [
        '<path d="' .
            'M18 2h-3a5 5 0 0 0-5 5v3' .
            'H7v4h3v8h4v-8h3l1-4h-4' .
            'V7a1 1 0 0 1 1-1h3z"/>',
    ]);
}

function icon_twitter(
    string $class = ""
): string {
    return icon_svg($class, [
        '<path d="' .
            'M23 3a10.9 10.9 0 0 1 ' .
            '-3.14 1.53A4.48 4.48 0 0 0 ' .
            '12 7.5v1A10.66 10.66 0 0 1 ' .
            '3 4s-4 9 5 13a11.64 11.64 ' .
            '0 0 1-7 2c9 5 20 0 20-11.5' .
            'a4.5 4.5 0 0 0-.08-.83' .
            'A7.72 7.72 0 0 0 23 3z"/>',
    ]);
}

function icon_instagram(
    string $class = ""
): string {
    return icon_svg($class, [
        '<rect x="2" y="2" width="20" ' .
            'height="20" rx="5" ry="5"/>',
        '<path d="' .
            'M16 11.37A4 4 0 1 1 ' .
            '12.63 8 4 4 0 0 1 16 11.37z"/>',
        '<line x1="17.5" y1="6.5" ' .
            'x2="17.5" y2="6.5"/>',
    ]);
}
